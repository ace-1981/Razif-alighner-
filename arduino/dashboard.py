...
HOMING_NAMES = {0: "FIND CENTER (fast)", 1: "BACKOFF", 2: "APPROACH (fine)", 3: "DONE"}
...
class AlignerConfig:
    ...
    seek_pwm_fast: int = 18
    seek_pwm_slow: int = 10
    seek_pwm_fine: int = 12
    limit_backoff_pwm: int = 55
...
class AlignerState:
    ...
    seek_dir_right: bool = False   # start LEFT (towards center sensor)
...
class AlignerSimulator:
    ...
    def reset_homing(self) -> None:
        ...
        self.st.seek_dir_right = False   # always start LEFT
        self.motor_stop()
    ...
    def _check_limit_backoff(self) -> bool:
        """If any limit is active, escape away from it. Returns True if backoff happened."""
        limL = self.st.limit_left_active
        limR = self.st.limit_right_active
        if not limL and not limR:
            return False
        wasDir = self.st.g_dir
        self.motor_stop()
        # escape direction: away from whichever limit is active
        if limL and not limR:
            escapeDir = +1
        elif limR and not limL:
            escapeDir = -1
        elif wasDir != 0:
            escapeDir = -wasDir
        else:
            escapeDir = +1
        if escapeDir > 0:
            self.motor_right(self.cfg.limit_backoff_pwm)
        else:
            self.motor_left(self.cfg.limit_backoff_pwm)
        self.st.seek_dir_right = not self.st.seek_dir_right
        return True
    ...
    def _homing_step(self) -> None:
        ...
        # Stage 0: Find center fast - move in seekDir until center ON
        ...
        # Stage 1: Backoff - move opposite seekDir SLOW until center OFF
        ...
        # Stage 2: Approach fine - move in seekDir VERY SLOW until center ON
...
class DashboardApp:
    ...
    def _build_ui(self) -> None:
        ...
        step_names = [
            (0, "1. FIND CENTER", "Move in seekDir until center ON"),
            (1, "2. BACKOFF", "Reverse slow until center OFF"),
            (2, "3. FINE APPROACH", "seekDir slow until center ON"),
            (3, "4. DONE", "Centered!"),
        ]
